...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_UP) == GLFW_PRESS) {
        if (mixValue + dMixValue <= 1) {
            mixValue += dMixValue;
        }
    }
    if (glfwGetKey(window, GLFW_KEY_DOWN) == GLFW_PRESS) {
        if (mixValue - dMixValue >= 0) {
            mixValue -= dMixValue;
        }
    }
}
...
int main() {
    ...
    Shader myShader("vertex_shader_for_textures.txt", "fragment_shader_for_textures_exercise4.txt");
