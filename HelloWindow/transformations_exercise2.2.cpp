...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        myShader.use();
        myShader.setMat4("transform", trans); //glUniformMatrix4fv(glGetUniformLocation(myShader.ID, "transform"), 1, GL_FALSE, glm::value_ptr(trans));
        ...
        trans = glm::scale(trans, glm::vec3(abs(sin(timeValue)), abs(sin(timeValue)), abs(sin(timeValue))));

        //set the matrix to the uniform
